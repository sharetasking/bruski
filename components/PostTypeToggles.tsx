...
const PostTypeToggles = ({onTypeChange, initial}: {onTypeChange: (type: MediaType) => void, initial:MediaType}) => {
  const [activePostType, setActivePostType] = useState(initial);

  onTypeChange(activePostType as MediaType);


  return (
    <div className="flex justify-center gap-2 w-fit p-2 rounded-3xl">
      <span
        className={`px-4 py-2 text-sm font-medium rounded-2xl focus:outline-none focus:ring-2 cursor-pointer focus:ring-indigo-500 transition-all duration-200 ${activePostType === 'TEXT' ? 'bg-primary/10 text-primary border-primary/5' : ' text-primary border-transparent hover:bg-primary/10 hover:text-primary/30 active:bg-primary/30'}`}
      ...
      <span
        className={`px-4 py-2 text-sm font-medium rounded-2xl focus:outline-none focus:ring-2 cursor-pointer focus:ring-indigo-500 transition-all duration-200 ${activePostType === 'CHALLENGE' ? 'bg-primary/10 text-primary border-primary/5' : ' text-primary border-transparent hover:bg-primary/10 hover:text-primary/30 active:bg-primary/30'}`}
