...
export const ModeToggle = ({className}:{className?:string}) => {
  ...
  const sunVisibility = theme === "light" ? "" : "hidden";
  const moonVisibility = theme === "dark" ? "" : "hidden";
  const systemVisibility = theme === "system" ? "" : "hidden";
...
<button onClick={toggleTheme} aria-label="toggle light and dark mode" className={"h-[2.2rem] w-[2.24rem] grow-0 shrink-0  items-center justify-center rounded-full bg-transparent "+(className?? "flex")}>
  <Sun className={"clickable h-[1.24rem] w-[1.24rem] transition-all light:text-black dark:text-white " + (theme == "light" ? "block" : "hidden")} />
  <Moon className={"clickable h-[1.2rem] w-[1.2rem] transition-all text-primary light:text-black dark:text-white " + (theme == "dark" ? "block" : "hidden")} />
  <svg width="20px" height="20px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" className={"clickable h-[1.2rem] w-[1.2rem] light:text-black dark:text-white " +  (theme == "system" || !theme ? "block" : "hidden")}>
